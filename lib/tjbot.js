...
function TJBot(hardware, configuration, credentials) {
    ...
    hardware.forEach(function(deviceice) {
        switch (device) {
            case 'camera':
                this._setupCamera()
                break;
    ...
    if (this.configuration.verboseLogging) {
        console.log("Hello from TJBot! (library version " + TJBot.prototype.version + ")");
    }
}
...
TJBot.prototype.defaultConfiguration = {
    ...
    listen: {
        microphoneDeviceId: 1, // TODO check this
...
TJBot.prototype._setupCamera = function() {
    ...
    this._camera = new Raspistill({
        width: self.configuration.see.camera.width,
        height: self.configuration.see.camera.height,
        noPreview: true,
        encoding: 'jpg',
        verticalFlip: self.configuration.see.camera.verticalFlip,
        horizontalFlip: self.configuration.see.camera.horizontalFlip
    });
}
...
TJBot.prototype._createServiceAPI = function(service, credentials) {
    ...
    switch (service) {
        case 'conversation':
            ...
            if (credentials['version_date'] == "") {
                credentials['version_date'] = "2016-07-11";
            }
            
            var ConversationV1 = require('watson-developer-cloud/conversation/v1');
            this._conversation = ConversationV1({
        ...
        case 'language_translator':
            ...
            this._languageTranslator = LanguageTranslatorV2({
                username: credentials['username'],
                password: credentials['password'],
                version: 'v2'
                // url: credentials['url']
        ...
        case 'tone_analyzer':
            ...
            this._toneAnalyzer = ToneAnalyzerV3({
                username: credentials['username'],
                password: credentials['password'],
                version: 'v3',
                version_date: credentials['version_date']
...
TJBot.prototype.analyzeTone = function(text) {
	...
	return new Promise(function(resolve, reject) {
		...
		self._toneAnalyzer.tone(params, function(err, tone) {
			if (err) {
				console.error("error: the tone_analyzer service returned an error. This may indicate you have exceeded your usage quota for the service.");
...
TJBot.prototype.converse = function(workspaceId, message, callback) {
    ...
    this._conversation.message(turn, function(err, conversationResponseObject) {
        if (err) {
            console.error("error: the conversation service returned an error. This may indicate you have exceeded your usage quota for the service.");
            console.error("raw error:");
            console.error(err);
        } else {
...
TJBot.prototype.listen = function(callback) {
    ...
    this._micTextStream = this.micInputStream.pipe(this._micRecognizeStream);
    this._micTextStream.setEncoding('utf8');
    
    // handle errors in the text stream
    this._micTextStream.on('error', function(err) {
        if (err) {
            console.error("error: the speech_to_text service returned an error. This may indicate you have exceeded your usage quota for the service.");
            console.error("raw error:");
            console.error(err);
...
TJBot.prototype.see = function() {
    ...
    return self._captureAndProcessImage(this._visualRecognition.classify, this.configuration.see.objectConfidenceThreshold);
}
...
TJBot.prototype.read = function() {
    this._assertCapability('see');
    return self._captureAndProcessImage(this._visualRecognition.recognizeText, this.configuration.see.textConfidenceThreshold);
}

/**
 * Capture and process an image from the camera.
 * 
 * @param {Function} processMethod The Watson function to call for processing the image.
 * @param {Double} threshold Recognition confidence threshold value to pass to the Watson service.
 */
TJBot.prototype._captureAndProcessImage = function(processMethod, threshold) {
    // capture 'this' context
    var self = this;
    ...
    return new Promise(function(resolve, reject) {
        temp.open('tjbot', function(err, info) {
            ...
            self._captureImage(filePath).then(function(buffer) {
                var params = {
                    images_file: fs.createReadStream(buffer),
                    threshold: threshold
                };
                
                processMethod(params, function(err, response) {
                    if (err) {
                        reject(err);
                    } else {
...
TJBot.prototype._captureImage = function(filePath) {
    ...
    var path = filePath.substring(0, str.lastIndexOf("/"));
    var name = filePath.substring(str.lastIndexOf("/") + 1);
...
TJBot.prototype.speak = function(message) {
    ...
    for (var i in this._ttsVoices) {
        if (this._ttsVoices[i]["language"] == this.configuration.language.speak && 
    ...
    if (this.configuration.verboseLogging) {
        console.info("> speaking with voice: " + selectedVoice);
    }
