...
TJBot.prototype._setupSpeaker = function() {
    ...
    // initialize the playerOptions to hold player parameters.
    // this variable can be updated by passing a similar structure to the .play(playerOptions) method.
    this.playerOptions = {
        filename: "",
        gain: 100,
        debug: false,
...
TJBot.prototype.listen = function(callback) {
    ...
    if (this.configuration.listen.customizationId) {
        recognizerParams.customization_id = this.configuration.listen.customizationId;
    }
 ...
 * @param {Object} playerOptions the play method can also take a json object that match parameters of the TJBot class variable playerOptions.
 *
 */
TJBot.prototype.play = function(soundFile) {
    // capture 'this' context
    var self = this;
    var currentOptions = Object.assign({}, self.playerOptions);

    if (typeof soundFile == 'string') {
        currentOptions.filename = soundFile
    } else {
        currentOptions = Object.assign({}, self.playerOptions, soundFile);
    }



    if (self.isPlaying) {
        if (self.configuration.speak.queueSpeech || currentOptions.queueSpeech) {
            winston.debug(this.speechQueue.length + " items in Queue. Queing audio file with parameter ", currentOptions);
            winston.verbose("You attempted to play audio while ", currentOptions.filename, " file was playing. Queing ");
            this.speechQueue.push(currentOptions) // add item to speech queue
        } else {
            winston.verbose("You attempted to play audio file ", currentOptions.filename, " while a file was playing. To enable Queuing, add `speak: {queueSpeech: true}` to tjconfig ");
        }
    } else {
        return new Promise(function(resolve, reject) {
            self.isPlaying = true;
            ...
            var player = new self._soundplayer(currentOptions);

            // play the audio
            player.play(currentOptions);

            winston.debug("Playing audio with parameters: ", currentOptions);
            ...
            player.on('complete', function() {
                ...
                if (self.speechQueue.length > 0) {
                    self.play(self.speechQueue.shift())
                }
