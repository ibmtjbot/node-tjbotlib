    ...
    */
    ...
    async converse(message) {
        ...
        try {
            ...
            winston.silly(`response from _assistant.message(): ${body}`);
    ...
    async recognizeObjectsInPhoto(filePath, classifierIds = []) {
        ...
        try {
            ...
            winston.silly(`response from _visualRecognition.classify() ${body}`);
    ...
    async speak(message) {
        ...
        if (this.configuration.speak.voice !== undefined) {
            ...
            if (!this._ttsVoices) {
                ...
                winston.silly(`response from _tts.listVoices(): ${body}`);
            ...
            const language = this.configuration.speak.language;
            const languageMatches = this._ttsVoices.filter((v) => {
            ...
            const gender = this.configuration.robot.gender;
            const languageAndGenderMatches = languageMatches.filter((v) => {
    ...
    async translate(text, sourceLanguage, targetLanguage) {
        ...
        try {
            ...
            winston.silly(`response from _languageTranslator.translate(): ${body}`);
    ...
    async identifyLanguage(text) {
        ...
        try {
            ...
            winston.silly(`response from _langaugeTranslator.identify(): ${body}`);
    ...
    async _loadLanguageTranslationModels() {
        ...
        try {
            ...
            winston.silly(`response from _languageTranslator.listModels(): ${body}`);
            models = body.result;
        } catch (err) {
            winston.error(`the ${TJBot.SERVICES.LANGUAGE_TRANSLATOR} service returned an error`, err);
            throw err;
