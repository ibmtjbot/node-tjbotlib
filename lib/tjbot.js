...
TJBot.prototype._setupMicrophone = function() {
    ...
    this._micInputStream.on('pauseComplete', function() {
        if (this.config.verboseLogging) console.log("> microphone paused");
    });
    ...
    this._micInputStream.on('processExitComplete', function() {
        if (this.config.verboseLogging) console.log("> microphone exit");
    });
...
TJBot.prototype.captureImage = function(filePath) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        cameraParams.push('-t', 0) // no time delay
        cameraParams.push('-np'); // no image preview .. makes capture 10x faster
