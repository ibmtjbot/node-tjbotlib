    ...
    switch (service) {
        ...
        case 'visual_recognition':
            ...
            if (credentials['version'] == "") {
                credentials['version'] = "2016-05-19"
            }
...
TJBot.prototype._setupMicrophone = function() {
    ...
    if (this.config.microphoneDeviceId) {
        micParams.device = this.config.microphoneDeviceId;
    }
...
TJBot.prototype._sendAudioWatsonSpeechtoText = function(callback) {
    ...
    if (this.config.verboseLogging) {
        console.log("> opening audiostream with watson speech to text")
    }
    // capture context
    var self = this;
        ...
        if (mode == "classify" || mode === undefined) {
            ...
            self._visualRecognition.recognizeText(visionParams, function(err, visionResponseObject) {
                ...
                if (err) {
                    ...
                    if (self.config.verboseLogging) {
                        console.log(" > ", visionResponseObject.images[0].text)
                    }
                    var response = {
                        "object": visionResponseObject,
...
TJBot.prototype.captureImage = function(filePath) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (self.config.cameraParams.vflip) {
            cameraParams.push('-vf');
        }
        if (self.config.cameraParams.hflip) {
