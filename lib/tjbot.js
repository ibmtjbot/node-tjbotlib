...
class TJBot {
    ...
    */
    ...
    constructor(configuration = {}, credentialsFile = '') {
        ...
        winston.silly(JSON.stringify(this.configuration));
    }
    ...
    */
        ...
        case TJBot.CAPABILITIES.SHINE:
            if (!this._led) {
                throw new Error(
                    ...
                    + 'Please check that you included the '
                    + `${TJBot.HARDWARE.LED} hardware in the TJBot initialize() method.`,
                );
            }
            break;
    ...
    listen(callback) {
        ...
        winston.silly(JSON.stringify(params));
    ...
    async speak(message) {
        ...
        if (this.configuration.speak.voice !== undefined) {
            ...
            winston.verbose(`selected ${voice} as the ${this.configuration.robot.gender} voice for ${this.configuration.speak.language} `);
        }
    ...
    /** 
     * Returns a list of languages that can TJBot can translate to.
     * @return {array} List of languages that TJBot can translate to (e.g. {"language": "af", "language_name": "Afrikaans"})
     */
    async translatableLanguages() {
        this._assertCapability(TJBot.CAPABILITIES.TRANSLATE);
        
        // lazy load of languages
        if (this._translatableLanguages === undefined) {
            winston.verbose('loading list of translatable languages...');
            this._translatableLanguages = await this._loadLanguages();
            
            winston.silly('translatable languages:');
            winston.silly(JSON.stringify(languages));
            winston.verbose('translatable languages loaded');
        }

        return this._translatableLanguages;
    }

    /**
     * Map between language name and the two-letter code used to identify the language in the language translator service.
     * @param {string} languageName Name of the language (e.g. "English")
     * @return {string} Language code (e.g. "en"). Returns undefined if no code is found for the given language.
     * @async
     */
    async languageCodeForLanguage(languageName) {
        this._assertCapability(TJBot.CAPABILITIES.TRANSLATE);

        // lazy load the language name -> code map
        if (this._languageCodes === undefined) {
            const languages = await this.translatableLanguages();
            this._languageCodes = {};
            languages.forEach((l) => {
                const name = l.language_name.toLowerCase();
                this._languageCodes[name] = l.language;
            });
        }
        
        const name = languageName.toLowerCase();
        return this._langaugeCodes[name];
    }
    ...
    /**
     * Load the list of languages to which TJBot can translate.
     */
    async _loadLanguages() {
        let languages;

        try {
            languages = await this._languageTranslator.listLanguages({});
        } catch (err) {
            winston.error(`the ${TJBot.SERVICES.LANGUAGE_TRANSLATOR} service returned an error`, err);
            throw err;
        }
