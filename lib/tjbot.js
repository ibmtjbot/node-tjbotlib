...
function TJBot(hardware, configuration, credentials) {
    ...
    // > twitter
    if (credentials != undefined && credentials.hasOwnProperty('twitter')) {
        var creds = credentials['twitter'];
        this._createServiceAPI('twitter', creds);
    }
...
TJBot.prototype.services = ['conversation', 'speech_to_text', 'text_to_speech', 'tone_analyzer', 'twitter'];
 ...
 *
 * @param {String} service The name of the service. Valid names are 'speech_to_text', 'text_to_speech', 'tone_analyzer', 'twitter'.
...
TJBot.prototype._createServiceAPI = function(service, credentials) {
    ...
    switch (service) {
        ...
        case 'twitter':
            assert(credentials.hasOwnProperty('consumer_key'), "credentials for the " + service + " service missing 'consumer_key'");
            assert(credentials.hasOwnProperty('consumer_secret'), "credentials for the " + service + " service missing 'consumer_secret'");
            assert(credentials.hasOwnProperty('access_token_key'), "credentials for the " + service + " service missing 'access_token_key'");
            assert(credentials.hasOwnProperty('access_token_secret'), "credentials for the " + service + " service missing 'access_token_secret'");
...
TJBot.prototype._assertCapability = function(capability) {
    switch (capability) {
        ...
        case 'tweet':
            if (!this._twitter) {
                throw new Error('TJBot is not configured to tweet.');
            }
            break;
...
TJBot.prototype.converse = function(workspaceId, message, callback) {
    ...
    this._conversation.message(turn, function(err, response) {
        if (err) {
            ...
            self._conversationContext[workspaceId] = response.context;
...
/**
 * See: Take picture and describe its content.
 *
 */
...
/**
 * [seeAsync describe image content]
 * @param  {[type]} mode [string classify | text ]
 * @return {[type]}      [description]
 */
TJBot.prototype.seeAsync = function(mode) {
    this._assertCapability('see');
    // capture 'this' context
    var self = this;
        ...
        if (mode == "classify" || mode === undefined) {
            ...
            self._visualRecognition.recognizeText(visionParams, function(err, visionResponseObject) {
                ...
                if (err) {
                    ...
                    //var visionClassifierResult = visionResponseObject.images[0].classifiers[0].classes
                    console.log(visionResponseObject, " === ", visionResponseObject.images[0].text)
...
TJBot.prototype.captureImage = function(filePath) {
    ...
    var self = this;
    return new Promise(function(resolve, reject) {
        var cameraparams = ['-w', '960', '-h', '720', '-o', filePath, '-t', '1'];
...
TJBot.prototype.speakAsync = function(message) {
    this._assertCapability('speak');
...
/**
 * Listen for spoken utterances containing the attention word.
 */
TJBot.prototype.listenWithAttentionWord = function(callback) {
    // make sure we can listen
    this._assertCapability('listen');
    ...
    // pass any data that begins with the attention word to the callback
    this._micTextStream.on('data', function(transcriptMessage) {
        if (transcriptMessage.startsWith(this.configuration.attentionWord)) {
            if (callback != undefined) {
                callback(transcriptMessage);
            }
        }
 ...
 */
TJBot.prototype.analyzeTone = function(text, callback) {
    ...
    this._toneAnalyzer.tone(params, function(err, tone) {
        if (err) {
...
/**
 * Listen for tweets with the given term.
 *
 * @param {String} term Term to listen for (e.g. "tjbot")
 */
TJBot.prototype.listenForTweetsWithTerm = function(term, callback) {
    this._assertCapability('tweet');

    if (this.configuration.verboseLogging) {
        console.log("TJBot monitoring twitter for " + term);
    }

    this._twitter.stream('statuses/filter', {
        track: term
    }, function(stream) {
